...
class Compares:
    ...
    def __init__(self, read_output="D:/Program_Files/Cache/cache/videos/缓存", write_enter="D:/Program_Files/Cache/cache/videos/输出", ph_types=None,
                 w_types=None, w_json_bool=True, new_makedirs=True):
        """
    ...
    def difference(self):
        ...
        if self.new_makedirs:
            ...
            for test_3_2 in self.wr_different:
                cache_re_read.append(Re_names(self.write_enter, self.read_output, test_3_2).re_flie())  # 临时变换为转码后的文件名
            ...
            for test_4 in reversed(self.write_output_files):  # 倒序读取文件夹列表
                if test_4 == self.write_enter:  # 跳过最上层的文件夹的删除
                    continue
                if test_4 not in cache_files_read:
                    print('删除文件夹：' + test_4)
...
if __name__ == '__main__':
    ...
    #  types = ['.wmv']
    types = [".mp4"]
    read_folder_1 = "D:/Program_Files/Cache/cache/videos/缓存"
    write_folder_1 = "D:/Program_Files/Cache/cache/videos/输出"
